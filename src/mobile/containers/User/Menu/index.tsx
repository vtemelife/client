...
const Menu: React.SFC<any> = () => {
  ...
  const { mutate: serverSignOut } = useMutate({
    verb: "POST",
    path: SERVER_URLS.SIGN_OUT.toPath()
  });

  const signOut = () => {
    serverSignOut({}).then((result: any) => {
      history.push({
        pathname: CLIENT_URLS.AUTH.SIGN_IN.toPath()
      });
  ...
  return (
    <div className="container-menu">
      ...
      <Header name={_("User menu")}>
        {isModerator() && (
          <div>
            <Link to={CLIENT_URLS.MODERATOR.MENU.toPath()}>
              <i
              ...
              <Popover id="popover-basic">
                <Popover.Content>
                  <ListGroup variant="flush">
                    <ListGroup.Item
                      onClick={() =>
                        changeLocale("ru", CLIENT_URLS.USER.MENU.toPath())
                      }
                    >
                    ...
                    <ListGroup.Item
                      onClick={() =>
                        changeLocale("en", CLIENT_URLS.USER.MENU.toPath())
                      }
                    >
      ...
      <div className="block">
        <Nav className="flex-column">
          <LinkContainer to={CLIENT_URLS.USER.FRIEND_LIST.toPath()}>
          ...
          <LinkContainer to={CLIENT_URLS.USER.MEDIA_FOLDER_LIST.toPath()}>
            <Nav.Link className="black-link">
              <i className="fa fa-photo" /> {_("Media folders")}
            </Nav.Link>
          </LinkContainer>
          <LinkContainer to={CLIENT_URLS.USER.GAME_LIST.toPath()}>
            <Nav.Link className="black-link">
              <i className="fa fa-gamepad" /> {_("Games")}
            </Nav.Link>
          </LinkContainer>
          <LinkContainer to={CLIENT_URLS.USER.BLACKLIST_LIST.toPath()}>
            <Nav.Link className="black-link">
      ...
      <div className="block">
        ...
        <Nav className="flex-column">
          ...
          <LinkContainer to={CLIENT_URLS.MAP.toPath()}>
            <Nav.Link className="black-link">
      ...
      <div className="block">
        <Nav className="flex-column">
          <LinkContainer to={CLIENT_URLS.USER.SETTINGS.toPath()}>
            <Nav.Link>
