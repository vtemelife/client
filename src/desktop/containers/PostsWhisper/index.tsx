...
class PostsWhisper extends React.PureComponent<IProps> {
  public renderTitle = (getParams: any) => {
    switch (getParams.theme) {
            ...
            value={
              getParams.theme
                ? {
                    ...
                    display: this.renderTitle({ theme: getParams.theme })
                  }
                : null
            }
            onChange={(target: any) =>
  ...
  public render() {
    return (
      <BlockPosts
        ...
        defaultGetParams={{ status: "approved", is_whisper: true }}
      />
    );
  }
}

export default PostsWhisper;
